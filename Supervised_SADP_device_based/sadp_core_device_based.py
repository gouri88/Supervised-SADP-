"""
sadp_core_device_based.py  (standalone — no other local imports required)
==========================================================================
Device-calibrated Supervised SADP.

This file is a fully self-contained drop-in replacement for sadp_core.py.
It includes every function and class from that module, plus three additions:

  DeviceKernel
      Loads real memtransistor conductance data from an Excel file, fits
      two splines (potentiation and depression branches), and exposes a
      vectorised  kappa -> delta_G/G0  mapping.

  DeviceSADPNetwork(SADPNetwork)
      Subclass with a single surgical change in update(): kappa1_eff is
      passed through DeviceKernel.transform() before the W1 weight update.
      Everything else -- forward pass, W2 update, regularisation -- is
      inherited unchanged.

  run_experiment_device(device_kernel, ...)
      Identical call signature to run_experiment(), replaces SADPNetwork
      with DeviceSADPNetwork, returns the same result dict plus three
      device-specific fields.

Usage
-----
    from sadp_core_device_based import DeviceKernel, run_experiment_device

    kernel = DeviceKernel(data_path="conductance_device_data.xlsx")
    result = run_experiment_device(kernel, dataset_name="mnist",
                                   encoding_type="cnn+poisson", k_shift=5)

Dependencies
------------
    numpy, scipy (UnivariateSpline), pandas, openpyxl, scikit-learn
    tensorflow  (only for cnn+poisson encoding)
    tqdm        (optional, for progress bars)

Keep this file and conductance_device_data.xlsx in the same directory,
or pass the full path via DeviceKernel(data_path=...).
"""
...
# ===========================================================================
# DEVICE KERNEL ADDITIONS
# ===========================================================================

from pathlib import Path
from typing import Any  # already imported above, harmless duplicate
from scipy.interpolate import UnivariateSpline


class DeviceKernel:
    """
    Memtransistor conductance-based SADP learning kernel.

    Replaces the linear kappa -> weight_update mapping with the real device
    response: kappa in [-1,1] -> delta_G/G0 measured from the device.

    Parameters
    ----------
    data_path : str or Path
        Path to the Excel file with conductance measurements.
        Default: 'conductance_device_data.xlsx' (same directory).
    sheet : str
        Sheet name inside the Excel file.  Default: 'N=200'.
    conductance_col : str
        Column name for conductance values.  Default: 'Conductance'.
    spline_s_pot : float
        Smoothing factor for the potentiation spline (kappa >= 0).
    spline_s_dep : float
        Smoothing factor for the depression spline (kappa < 0).
    normalise : bool
        If True, rescale so that RMS of f_device over [-1,1] equals 1,
        keeping eta_in comparable to the standard SADP run.
    """

    def __init__(
        self,
        data_path: str = "conductance_device_data.xlsx",
        sheet: str = "N=200",
        conductance_col: str = "Conductance",
        spline_s_pot: float = 0.1,
        spline_s_dep: float = 0.01,
        normalise: bool = False,
    ):
        self.data_path       = Path(data_path)
        self.sheet           = sheet
        self.conductance_col = conductance_col
        self.spline_s_pot    = spline_s_pot
        self.spline_s_dep    = spline_s_dep
        self.normalise       = normalise

        self._spline_pot  = None
        self._spline_dep  = None
        self._pot_domain  = (0.0, 1.0)
        self._dep_domain  = (-1.0, 0.0)
    ...
    def _load_and_fit(self):
        if not self.data_path.exists():
            raise FileNotFoundError(
                f"Conductance file not found: {self.data_path}\n"
                "Pass the correct path via DeviceKernel(data_path=...) "
                "or --device-data on the command line."
            )
        logger.info("DeviceKernel: loading %s | sheet=%s",
                    self.data_path.name, self.sheet)

        import pandas as _pd
        df  = _pd.read_excel(self.data_path, sheet_name=self.sheet)
        G   = df[self.conductance_col].dropna().values.astype(np.float64)
        mid = len(G) // 2
        G_pot, G_dep = G[:mid], G[mid: mid * 2]

        dG_pot = np.diff(G_pot) / (G_pot[:-1] + 1e-7)
        dG_dep = np.diff(G_dep) / (G_dep[:-1] + 1e-7)

        tp = np.linspace(1.0,  0.0, len(dG_pot))
        td = np.linspace(-1.0, 0.0, len(dG_dep))

        sp = np.argsort(tp); sd = np.argsort(td)
        kp, dgp = tp[sp], dG_pot[sp]
        kd, dgd = td[sd], dG_dep[sd]

        self._spline_pot = UnivariateSpline(kp, dgp, s=self.spline_s_pot)
        self._spline_dep = UnivariateSpline(kd, dgd, s=self.spline_s_dep)
        self._pot_domain = (float(kp.min()), float(kp.max()))
        self._dep_domain = (float(kd.min()), float(kd.max()))

        if self.normalise:
            grid = np.linspace(-1.0, 1.0, 500)
            vals = np.where(
                grid >= 0,
                self._spline_pot(np.clip(grid, *self._pot_domain)),
                self._spline_dep(np.clip(grid, *self._dep_domain))
            )
            rms = float(np.sqrt(np.mean(vals ** 2)))
            self._norm_scale = 1.0 / (rms + 1e-12)

        logger.info(
            "DeviceKernel fitted | pot=[%.3f,%.3f] dep=[%.3f,%.3f] | "
            "n_pot=%d n_dep=%d | normalise=%s | norm_scale=%.4f",
            *self._pot_domain, *self._dep_domain,
            len(dG_pot), len(dG_dep), self.normalise, self._norm_scale
        )

    def transform(self, kappa: np.ndarray) -> np.ndarray:
        """Vectorised kappa -> delta_G/G0 mapping."""
        out = np.zeros_like(kappa, dtype=np.float32)
        mp  = kappa >= 0.0
        md  = kappa <  0.0
        if mp.any():
            kp = np.clip(kappa[mp].astype(np.float64),
                         self._pot_domain[0], self._pot_domain[1])
            out[mp] = self._spline_pot(kp).astype(np.float32)
        if md.any():
            kd = np.clip(kappa[md].astype(np.float64),
                         self._dep_domain[0], self._dep_domain[1])
            out[md] = self._spline_dep(kd).astype(np.float32)
        return out * self._norm_scale

    def plot(self, save_path: Optional[str] = None, show: bool = True):
        """Plot raw data and fitted splines for both branches."""
        try:
            import matplotlib.pyplot as plt
            import pandas as _pd
        except ImportError:
            logger.warning("matplotlib/pandas not available for plot.")
            return
        df  = _pd.read_excel(self.data_path, sheet_name=self.sheet)
        G   = df[self.conductance_col].dropna().values.astype(np.float64)
        mid = len(G) // 2
        G_p, G_d = G[:mid], G[mid: mid * 2]
        dgp = np.diff(G_p) / (G_p[:-1] + 1e-7)
        dgd = np.diff(G_d) / (G_d[:-1] + 1e-7)
        tp  = np.linspace(1.0, 0.0, len(dgp))
        td  = np.linspace(-1.0, 0.0, len(dgd))
        fp  = np.linspace(*self._pot_domain, 300)
        fd  = np.linspace(*self._dep_domain, 300)

        plt.figure(figsize=(7, 5))
        plt.scatter(td, dgd * self._norm_scale,
                    color="red",  s=20, label="Depression data (κ < 0)")
        plt.scatter(tp, dgp * self._norm_scale,
                    color="blue", s=20, label="Potentiation data (κ ≥ 0)")
        plt.plot(fd, self._spline_dep(fd) * self._norm_scale,
                 "c-", lw=2.5, label="Depression spline")
        plt.plot(fp, self._spline_pot(fp) * self._norm_scale,
                 "m-", lw=2.5, label="Potentiation spline")
        plt.axhline(0, color="gray", ls="--", lw=0.8)
        plt.xlabel("κ  (spike agreement)", fontsize=14)
        plt.ylabel("ΔG / G₀  (device response)", fontsize=14)
        plt.title("Memtransistor SADP Kernel  f_device(κ)", fontsize=14)
        plt.legend(fontsize=11)
        plt.tight_layout()
        if save_path:
            plt.savefig(save_path, dpi=300)
            logger.info("Kernel plot saved -> %s", save_path)
        if show:
            plt.show()
        plt.close()

    def __repr__(self):
        return (f"DeviceKernel(path={self.data_path.name!r}, "
                f"sheet={self.sheet!r}, normalise={self.normalise})")


class DeviceSADPNetwork(SADPNetwork):
    """
    SADPNetwork with device-calibrated weight update.

    Identical to SADPNetwork except that kappa1_eff is passed through
    DeviceKernel.transform() before the W1 einsum — one line changed.
    """

    def __init__(self, config: SADPConfig, kernel: DeviceKernel):
        super().__init__(config)
        self._kernel = kernel

    def update(self, x_batch_feats, y_batch, spikes_h, spikes_h2, spikes_o):
        cfg       = self.cfg
        B, T, _   = spikes_o.shape
        ...
        # W2: supervised Hebbian output update (unchanged)
        errors  = targets[:, None, :] - spikes_o
        pre_out = spikes_h2 if cfg.architecture == "2SADP" else spikes_h
        dW2 = np.einsum("bti,btj->ij", pre_out, errors) / B
        self.W2 += cfg.eta_out * dW2
        ...
        # W1: device-calibrated SADP (THE only change vs SADPNetwork)
        kappa1     = compute_shifted_kappa(spikes_h, target_spikes,
                                           k_shift=cfg.k_shift,
                                           weight_by_overlap=cfg.weight_by_overlap,
                                           eps=cfg.kappa_eps)
        kappa1_eff = kappa1 if r is None else kappa1 * r[:, None]
        kappa1_eff = self._kernel.transform(kappa1_eff)   # ← device kernel
        dW1 = np.einsum("bi,bj->ij", x_batch_feats, kappa1_eff) / B
        self.W1 += cfg.eta_in * dW1

        # W1_2 for 2SADP (also device-calibrated)
        if cfg.architecture == "2SADP" and spikes_h2 is not None:
            kappa2     = compute_shifted_kappa(spikes_h2, target_spikes,
                                               k_shift=cfg.k_shift,
                                               weight_by_overlap=cfg.weight_by_overlap,
                                               eps=cfg.kappa_eps)
            kappa2_eff = kappa2 if r is None else kappa2 * r[:, None]
            kappa2_eff = self._kernel.transform(kappa2_eff)
            dW1_2 = np.einsum("bi,bj->ij",
                              np.mean(spikes_h, axis=1), kappa2_eff) / B
            self.W1_2 += cfg.eta_in * dW1_2
            self.W1_2 *= cfg.decay
            self.W1_2 /= (np.linalg.norm(self.W1_2, axis=0, keepdims=True)
                          + cfg.norm_eps)

        # Decay & normalise (unchanged)
        self.W1 *= cfg.decay
        self.W2 *= cfg.decay
        self.W1 /= (np.linalg.norm(self.W1, axis=0, keepdims=True) + cfg.norm_eps)
        np.clip(self.W2, -cfg.clip_w2, cfg.clip_w2, out=self.W2)
...
def run_experiment_device(
    ...
    """
    Device-calibrated SADP experiment.  Identical to run_experiment() except
    SADPNetwork is replaced by DeviceSADPNetwork.  Returns the same result
    dict plus Device_Kernel_Path, Device_Kernel_Sheet, Device_Normalised.
    """
    if data is None:
        if dataset_name is None:
            raise ValueError("Provide either dataset_name or data=...")
    ...
    logger.info("[device] dataset=%s encoding=%s arch=%s T=%d k=%d kernel=%s",
                dataset_name.upper(), encoding_type, architecture,
                T, k_shift, repr(device_kernel))

    set_seed(seed)
    n_classes = len(np.unique(y_train))
    cache_key = (dataset_name, encoding_type, feature_dim, encoder_epochs,
                 batch_size_encoder, pretrain_encoder,
                 classical_grid, classical_n_bins, include_color_moments, seed)
    ...
    if feature_cache is not None and cache_key in feature_cache:
        ...
        train_feats, test_feats = extract_features_for_dataset(
            x_train, x_test, y_train,
            encoding_type=encoding_type, feature_dim=feature_dim,
            n_classes=n_classes, encoder_epochs=encoder_epochs,
            batch_size_encoder=batch_size_encoder,
            pretrain_encoder=pretrain_encoder,
            classical_grid=classical_grid, classical_n_bins=classical_n_bins,
            include_color_moments=include_color_moments,
            seed=seed, input_shape=input_shape,
        )
        if feature_cache is not None:
            feature_cache[cache_key] = (train_feats, test_feats)
    feature_time = time.time() - t_feat0
    ...
    cfg = SADPConfig(
        ...
        reward_mode=reward_mode, reward_scale=reward_scale,
        reward_baseline=reward_baseline,
        reward_baseline_decay=reward_baseline_decay, seed=seed,
    )
    net = DeviceSADPNetwork(cfg, kernel=device_kernel)

    t_train0 = time.time()
    history  = train_snn(net, train_feats, y_train,
    ...
    metrics = evaluate_snn(net, test_feats, y_test,
                           batch_size=eval_batch_size, seed=seed,
                           show_progress=show_progress)

    result = {
        "Dataset": dataset_name.upper(), "Encoding": encoding_type,
        "Architecture": architecture, "Timestep": T, "K_shift": k_shift,
        "Weight_by_overlap": weight_by_overlap, "Reward_Mode": reward_mode,
        "Reward_Scale": reward_scale, "Reward_Baseline": reward_baseline,
        "Nin": train_feats.shape[1], "Nhid": Nhid,
        "Eta_In": eta_in, "Eta_Out": eta_out, "Seed": seed,
        "Final_Train_Accuracy":   history["epoch_accuracy"][-1]
                                  if history["epoch_accuracy"] else None,
        "Eval_Accuracy":          metrics["accuracy"],
        "Precision":              metrics["precision"],
        "Recall":                 metrics["recall"],
        "F1_score":               metrics["f1"],
        ...
        "Epoch_Eval_Accuracies":  history["epoch_eval_accuracy"],
        # device-specific
        "Device_Kernel_Path":     str(device_kernel.data_path),
        "Device_Kernel_Sheet":    device_kernel.sheet,
        "Device_Normalised":      device_kernel.normalise,
    }
    logger.info(
        "[device] RESULT | %s | %s | k=%d | reward=%s "
        "-> eval_acc=%.4f f1=%.4f (%.2fs/epoch)",
        result["Dataset"], encoding_type, k_shift, reward_mode,
        result["Eval_Accuracy"], result["F1_score"], avg_time_per_epoch,
    )
    return result
